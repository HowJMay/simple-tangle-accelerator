 ...
 *
 * Explore transaction hash information with given transaction hashes. This
 * would return whole transaction object details in transaction_array_t instead
 * of raw trytes, includes address, value, timestamp, mwm, nonce...
 *
 * @param[in] service IRI node end point service
 ...
 *
 * We can get a bundle with any address in the bundle. Moreover, because the
 * channel ID in MAM is actually the address of a transaction, we can use this
 * function to search which bundle contains the message transaction we want to
 * fetch.
 *
 * @param[in] service IRI node end point service
 * @param[in] addr searched address in flex_trit_t
 * @param[in] bundle pointer of bundle object that will contain the MAM
 * transacitons
 *
 * @return
 * - SC_OK on success
