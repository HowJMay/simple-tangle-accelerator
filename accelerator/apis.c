...
status_t api_mam_send_message(const iota_config_t* const tangle, const iota_client_service_t* const service,
  ...
  // Creating channel
  // TODO: TA only supports mss depth under 2. Larger than this would result in
  // wasting too much time generate keys. With depth of 2, user can send a
  // bundle with  3 transaction messages filled (last one is for header).
